...
<div class="company-holder">
	<div style="display: inline-block;" class="{killer.children != null ? 'companies-group-image-holder' : 'companies-image-holder'}">
			<div class="grid place-items-center mt-2 hover:cursor-pointer" on:mouseenter={hover} on:mouseleave={unhover} on:click={click}>
				...
				<div class="p-6 text-left bg-background-color" style="height: 100%; width: 100%; grid-area: 1/1/1/1; opacity: {flipped ? '100%' : '0%'}; min-height: {killer.pic != null ? '16rem' : '8rem'};">
					<div class="p-2 m-1 font-bold inline-block bg-white rounded-lg">{killer.name}</div>
					{#if killer.all_brands != null}<div><a class="link p-2 m-1 font-bold bg-ukraine-yellow rounded-lg inline-block" target="_blank" href={killer.all_brands}
					>view all brands</a
					></div>{/if}
					<div class="p-2 m-1 bg-ukraine-blue text-white font-bold rounded-lg inline-block">{killer.area}</div>
					<div class="flex">
	...
	{#if killer.children != null}
		<div class="place-items-center companies-children-grid">
